...
# Capture all state in a single tmux command
{
    ...
    tmux list-panes -a -F "#{session_name}${TAB}#{window_index}${TAB}#{window_name}${TAB}#{window_layout}${TAB}#{pane_index}${TAB}#{pane_current_path}${TAB}#{window_active}${TAB}#{pane_active}"
} > "$TMP_FILE"
